...
class FilenameGenerator:
    ...
    def generate_filename(
        ...
        try:
            ...
            return self._generate_fallback_filename(
                content, extension, include_timestamp
            )
    ...
    def _generate_with_litellm(self, content: str) -> str:
        ...
        response = litellm.completion(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": f"以下の話の内容から適切なファイル名を作成してください:\n\n{content}",
                },
            ],
    ...
    def _generate_fallback_filename(
        self, content: str, extension: str, include_timestamp: bool
    ) -> str:
...
Examples:
  ...
  %(prog)s -f script.txt -e py --max-length 30 --provider openai
    ...
    text_group.add_argument(
        "-t", "--text", help="Text content to generate filename from"
    )
    text_group.add_argument(
        "-f",
        "--file",
    ...
    # 出力設定
    parser.add_argument(
        "-e", "--extension", default="txt", help="File extension (default: txt)"
    )
    parser.add_argument(
        "--no-timestamp",
        action="store_true",
    ...
    parser.add_argument(
        "--api-key",
        help="API key (can also be set via OPENAI_API_KEY or GEMINI_API_KEY environment variable)",
    )
    ...
    try:
        # ファイル名生成器を初期化
        generator = FilenameGenerator(provider=args.provider, api_key=args.api_key)
