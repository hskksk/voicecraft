...
class GeminiSpeechSynthesizer(SpeechSynthesizer):
    ...
    def _validate_voices(self):
        ...
        if not validate_voice(self.voice):
            ...
            raise ValueError(
                f"Unsupported voice '{self.voice}'. Available voices: {available_voices}"
            )
        ...
        if self.multi_speaker and self.speakers:
            for speaker in self.speakers:
                ...
                if not validate_voice(voice_name):
                    available_voices = list(GEMINI_VOICES.keys())
                    raise ValueError(
                        f"Unsupported voice '{voice_name}' for speaker. Available voices: {available_voices}"
                    )
    ...
    def list_available_voices(self) -> Dict[str, Dict[str, str]]:
        ...
        return {
            name: {"characteristic": info.characteristic, "category": info.category}
        ...
        for speaker in self.speakers:
            ...
            if name and description:
                ...
                if voice_characteristic:
                    speaker_instruction += (
                        f" (Voice characteristic: {voice_characteristic})"
                    )
    ...
    def add_speaker(self, speaker_name: str, voice_name: str, description: str = ""):
        ...
        speaker = types.Speaker(
            name=speaker_name, voice_name=voice_name, description=description
        )
